    ...
    public class OptionsWrapper<T>
    {
        ...
        private static void LoadOptions()
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    UnityEngine.Debug.LogWarning("[OptionsWrapper] Failed to load options, using defaults: " + e.Message);
                }
                SaveOptions();
            }
        }
        ...
        private static string GetFileName()
        {
            ...
            var attrs = type.GetCustomAttributes(typeof(OptionsAttribute), false);
            var modsSettingsPath = Path.Combine(DataLocation.localApplicationData, "ModsSettings");
            if (!Directory.Exists(modsSettingsPath))
            {
                Directory.CreateDirectory(modsSettingsPath);
            }
            var fileName = Path.Combine(modsSettingsPath, ((OptionsAttribute) attrs[0]).FileName);
